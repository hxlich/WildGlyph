...
WG_Event_FrameLock_SideEffectRestoreSize = function(store, args)
	local sw = getRealScreenWidth()
	local sh = getRealScreenheight()

	if store == nil then WG_Lib_Print.Warning("Store is nill") end
...
WG_Event_FrameLock_Init = function()
	...
	openWarning.Text:SetText(WILDGLYPH_T.UI.WarnUnlocked)
	openWarning.Text:SetTextColor(1, 1, 1)
	openWarning:SetAllPoints(UIParent)
	if Wild_Glyph_Store.IsLockedFrames
...
local addFrameMoveable = function(frame)
	if not Wild_Glyph_Store.IsLockedFrames then
...
local lockFrames = function()
	...
	for _k, v in _G.WG_Modules do
		if Wild_Glyph_Store.ModuleEnabled[v.Id] then v.OnInterfaceLock() end
	end
end
local unlockFrames = function()
	...
	for _k, v in _G.WG_Modules do
		if Wild_Glyph_Store.ModuleEnabled[v.Id] then v.OnInterfaceUnlock() end
	end
end

WG_Event_FrameLock_Set = function(isChecked)
	Wild_Glyph_Store.IsLockedFrames = isChecked
...
WG_Event_FrameLock_SideEffectMakeMoveable = function(f, store)
	...
	f:SetScript("OnMouseDown", function()
		if not Wild_Glyph_Store.IsLockedFrames then f:StartMoving() end
	end)
...
WG_Event_FrameLock_SideEffectMakeResizeable = function(frame, store, args)
